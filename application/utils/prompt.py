...
Pay attention to use today() function to get the current date, if the question involves "today". Pay attention to adapted to the table field type. Please follow the clickhouse syntax or function case specifications.If the field alias contains Chinese characters, please use double quotes to Wrap it.
When generating SQL related to dates and times, please strictly use the Clickhouse SQL Functions listed in the following md tables contents in <data_time_function_list>:
<data_time_function_list>
| Function | returns |
|---|---|
| toYear(date) | UInt16 |
| toMonth(date) | UInt8 |
| toDayOfMonth(date) | UInt8 |
| toHour(date) | UInt8 |
| toMinute(date) | UInt8 |
| toSecond(date) | UInt8 |
| toDate(value) | Date |
| toDateTime(value) | DateTime |
| toUnixTimestamp(date) | UInt32 |
| fromUnixTimestamp(timestamp) | DateTime |
| today() | Date |
| yesterday() | Date |
| formatDateTime(date, format) | String |
| toStartOfYear(date) | DateTime |
| toStartOfMonth(date) | DateTime |
| toStartOfWeek(date, mode) | DateTime |
| toStartOfDay(date) | DateTime |
| dateDiff(unit, startdate, enddate) | Int32 |
| dateAdd(unit, interval, date) | DateTime |
| dateSub(unit, interval, date) | DateTime |
</data_time_function_list>
""".format(top_k=TOP_K)
